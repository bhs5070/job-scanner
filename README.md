...
GPT-4o를 Judge 모델로 사용하여 합성 쿼리 20건에 대해 10개 메트릭을 자동 평가한 결과입니다.

| 메트릭 | 점수 | 설명 |
|--------|------|------|
| Relevance | 0.944 | 질문-답변 관련성 |
| Groundedness | 0.889 | 검색 데이터에 근거한 답변 |
| Helpfulness | 0.833 | 취업 준비생에게 실용적 도움 |
| Faithfulness | 0.889 | Hallucination 없음 |
| Answer Completeness | 0.944 | 질문의 모든 부분에 답변 |
| Retrieval Precision | 0.972 | 검색된 공고의 관련성 |
| Retrieval MRR | 0.972 | 가장 관련 있는 결과의 순위 |
| Context Relevance | 0.944 | 검색 컨텍스트 적합성 |
| Routing Accuracy | 0.722 | 의도 분류 정확도 |
| **평균** | **0.902** | |

> 합성 쿼리 18건 기준. 응답 생성: GPT-4o-mini (비용 효율), 평가: GPT-4o (엄격한 판정)으로 모델을 분리하여 평가 신뢰도를 확보했습니다.
